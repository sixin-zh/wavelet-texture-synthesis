...
class Discriminator(nn.Module):
    ...
    def __init__(self, ndf, nDep, opt, ncIn=3, bSigm=True):
...
class NetG(nn.Module):
    ...
    def __init__(self, ngf, nDep, nz,opt,nc=3):
...
class NetED(nn.Module):
    ...
    def __init__(self, ngf, nDep, bfirstNoise, opt,\
                 nz=0, Ubottleneck=-1, nc=3, ncIn=None,\
                 bTanh=True, lessD=0,bCopyIn=False):
...
class NetUskip(nn.Module):
    ...
    # @param bSkip turns skip connections btw encoder and decoder off
    # @param bTanh turns nonlinearity on and off
   def __init__(self, ngf, nDep, bfirstNoise,\
                nz=0, Ubottleneck=-1, nc=3, ncIn=None,\
                bSkip=True, bTanh =True, bCopyIn=False):
   ...
   def forward(self, input1,bfirstNoise,input2=None):
...
class NetU_MultiScale(nn.Module):
    ...
    def forward(self, input1, bfirstNoise,input2=None,M=None):
        if bfirstNoise and input2 is not None:
            x = torch.cat([input1, nn.functional.upsample(input2, scale_factor=2 ** self.nDep,mode='bilinear')], 1)
            input2 = None
        else:
