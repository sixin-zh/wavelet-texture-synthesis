...
from utils_arch_stationary import DiscriminatorInv_CirPoolSigm, LinIdwt2D
...
parser.add_argument('-J','--filter_J',type=int, default = 4)
parser.add_argument('-fs','--filter_size',type=int, default = 32) # filter size of Generator
parser.add_argument('-wave','--wavelet', type=str, default='db7')
...
params = {'fs':filter_size, 'wave':wave, 'J': J, 'nDepD':nDepD,'ndf':opt.ndf,\
...
if opt.beta1 > 0:
    ...
    print('use sgd')
    optimizerD = optim.SGD(netD.parameters(), lr=opt.lrD)
