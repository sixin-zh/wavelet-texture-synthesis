...
# use wph model A moments, + delta j and l moments
...
from kymatio.phaseharmonics2d.phase_harmonics_k_bump_modelA_djl \
    import PhaseHarmonics2d as wphshift2d_supp
...
params = {'J':args.scatJ, 'L':args.scatL,'fs':args.filter_size,\
          'dn':args.delta_n,'dj':args.delta_j,'dl':args.delta_l,\
          'its':args.max_iter,'bs':args.batch_size,'factr':args.factr,\
          'runid':args.run_id,'init':args.init,'gpu':args.gpu,\
          'loaddir':hash_str2int2(loaddir)}
outdir = './ckpt/' + args.dataname + '_linIdwt2d_modelA_djl'
mkdir(outdir)
...
# load discriminator network 2
nb_chunks = 1
for chunk_id in range(nb_chunks):
    wph_op = wphshift2d_supp(M,N,J,L,delta_j,delta_l,nb_chunks,chunk_id,submean=subm,stdnorm=stdn)
    if chunk_id ==0:
        total_nbcov += wph_op.nbcov
    if gpu:
        wph_op = wph_op.cuda()
...
def obj_func(xbatch,wph_ops,factr_ops,Sims):
    ...
    for op_id in range(len(wph_ops)):
        ...
        #loss_t.backward(retain_graph=False) # chunk
        loss = loss + loss_t
    return loss
...
copyfile('./train_linIdwt2d_periodic_modelA_djl.py', outdir + '/code.txt')
