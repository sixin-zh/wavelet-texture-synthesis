...
% estimate power spectrum from training data by maxent model A
% load hXrec
J = 5;
L = 4;
dn = 1;
...
for fold = 1:Kfold    
    kid = fold;
    [ofile] = get_ofile(name,model,J,L,dn,Kfold,kid)
    load(ofile);
    hatKr = hXrec;
...
function [ofile,Krecs] = get_ofile(name,model,J,L,dn,Kfold,ks)
    ofile = sprintf('../kymatio_wph3/projetLip1/results_acha/pwregress_maxent_bumps2d_dj0_nor_Delta%d_%s_J%d_L%d_K%d_m1_kb%d',dn,name,J,L,Kfold,ks);   
end
