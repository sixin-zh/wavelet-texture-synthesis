# TRAIN g2d linear GENERATOR, 2D PERIODIC
...
from utils_arch_lin import Pyramid2D_lin
...
parser.add_argument('-ch','--ch_step',type=int, default = 1)
...
outdir = './ckpt/' + args.dataname + '_g2d_lin_modelA'
mkdir(outdir)
...
if loaddir is None:
    gen = Pyramid2D_lin(ch_in=n_input_ch, ch_step=ch_step)
else:
...
for i in range(batch_size):
    zk = [torch.randn(1,n_input_ch,int(szk),int(szk)) for szk in sz]
    if gpu:
...
copyfile('./train_g2d_lin_periodic_modelA.py', outdir + '/code.txt')
